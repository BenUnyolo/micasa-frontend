const innerContainer = document.querySelector("#inner-container-reference");

const featuredHouses = document.querySelector("#featured-houses");
const featuredHousesContainer = document.querySelector(
  "#featured-houses-container"
);
const houseButton = document.querySelector("#house-button");
const villaButton = document.querySelector("#villa-button");
const apartmentButton = document.querySelector("#apartment-button");

// PADDING LEFT FOR FEATURED HOUSES CARDS
const updateFeaturedHousingPadding = () => {
  // set padding left to offset left of inner container reference element
  featuredHousesContainer.style.paddingLeft = innerContainer.offsetLeft + "px";
};
// run initial function
updateFeaturedHousingPadding();
// window resize event to update value
window.addEventListener("resize", updateFeaturedHousingPadding);
...
villaData = [
  {
    tag: "popular",
    image: "assets/images/house_cards/villa_1.jpeg",
    location: "Verdun, Montreal",
    bedrooms: 2,
    wifi: true,
    price: "100,000",
...
apartmentData = [
  {
    tag: "popular",
    image: "assets/images/house_cards/apartment_1.jpeg",
    location: "St Henri, Montreal",
    bedrooms: 1,
    wifi: true,
    price: "800,000",
...
const generateCard = (cardData) => {
  let { tag, image, location, bedrooms, wifi, price, timeframe, link } =
    cardData;
  return `<div class="flex-none w-full px-4 sm:w-1/2 md:w-1/3 lg:w-1/4">
          <div class="border p-2.5 rounded-lg bg-white w-full h-full">
            <!-- top section -->
            <div class="relative">
              <!-- image -->
              <img
              ...
              <!-- tag -->
              ${
                // shows relevant tag if present
                tag && tag === "new-house"
                  ? '<div class="house-card__tag house-card__tag--new-house">New House</div>'
                  : tag === "popular"
                  ? '<div class="house-card__tag house-card__tag--popular">Popular</div>'
                  : ""
              }
            </div>
            <!-- title section -->
            <div class="flex justify-between mt-3.5">
              <!-- title -->
              <h4 class="inline-flex items-center text-grey-800 min-h-14">
              ${location}
              </h4>
              <!-- icons -->
              <div class="inline-flex items-center flex-none">
                ...
                <div class="inline-block">
                  ...
                  <div class="text-center text-xxxs">${bedrooms} Bedroom</div>
                </div>
                <!-- wifi -->
                ${
                  wifi
            ...
            <div class="mt-3.5">
              <p>
                ...
                ><span class="inline-block text-xl">${price}</span
            ...
            <div class="flex justify-center w-full mt-5 text-white">
              <a href="${link}" class="px-4 md:px-12 btn-standard"
                >View More
                <svg
                  width="18"
                  height="11"
...
// setting up cards html
let houseCards = "";
houseData.forEach((card) => {
  console.log(card);
  houseCards = houseCards + generateCard(card);
});
houseCards = houseCards + houseCards;
let villaCards = "";
let villaCard = generateCard(villaData[0]);
let apartmentCards = "";
let apartmentCard = generateCard(apartmentData[0]);
for (let i = 0; i < 6; i++) {
  villaCards = villaCards + villaCard;
  apartmentCards = apartmentCards + apartmentCard;
}

// setting initial html
featuredHouses.innerHTML = houseCards + houseCards;

// functions for button clicked
const onClickHousesButton = (e) => {
  houseButton.classList.remove("btn-rounded--orange");
  villaButton.classList.remove("btn-rounded--orange");
  apartmentButton.classList.remove("btn-rounded--orange");
  switch (e.target.dataset.type) {
    case "house":
      featuredHouses.innerHTML = houseCards + houseCards;
      houseButton.classList.add("btn-rounded--orange");
      break;
    case "villa":
      featuredHouses.innerHTML = villaCards;
      villaButton.classList.add("btn-rounded--orange");
      break;
    case "apartment":
      apartmentButton.classList.add("btn-rounded--orange");
      featuredHouses.innerHTML = apartmentCards;
      break;
    default:
      featuredHouses.innerHTML = houseCards;
  }
};

// event listeners for buttons
houseButton.addEventListener("click", onClickHousesButton);
villaButton.addEventListener("click", onClickHousesButton);
apartmentButton.addEventListener("click", onClickHousesButton);
